...
$stmt = $pdo->prepare("SELECT id as package_id, name, price, daily_profit, duration_days, daily_return_percentage, is_active FROM mining_packages WHERE is_active = TRUE ORDER BY price ASC");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['purchase_package'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (!$package_id) {
            ...
            $stmt = $pdo->prepare("SELECT id as package_id, name, price, duration_days, is_active FROM mining_packages WHERE id = ?");
