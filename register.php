...
verify_csrf_token();
// PHPMailer removed as requested to only use PHP and SQL
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        $form_data['full_name'] = htmlspecialchars($_POST['full_name'] ?? '', ENT_QUOTES, 'UTF-8');
        $form_data['email'] = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL);
        $form_data['referral_code'] = htmlspecialchars($_POST['referral_code'] ?? '', ENT_QUOTES, 'UTF-8');
            ...
            if ($stmt->fetchColumn() > 0) {
                ...
                if (empty($error)) {
                    ...
                    try {
                        ...
                        // Verification email simulated using standard mail() or just logged
                        $verification_link = "http://" . $_SERVER['HTTP_HOST'] . "/verify_email.php?token=" . urlencode($verification_token);
                        $subject = 'Verify Your CryptoMiner ERP Account';
                        $message = "Welcome to CryptoMiner ERP!\n\nPlease verify your email by clicking: $verification_link";
                        // mail($form_data['email'], $subject, $message); // Uncomment if mail() is configured
