...
if (!isset($_GET['token']) || empty($_GET['token'])) {
    ...
    $stmt = $pdo->prepare("SELECT id, email, full_name, verification_token_expires, account_status FROM users WHERE verification_token = ?");
    $stmt->execute([$token]);
    $user = $stmt->fetch();
    ...
    if (!$user) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("UPDATE users SET account_status = 'active', verification_token = NULL, verification_token_expires = NULL WHERE id = ?");
            ...
            $stmt->execute([$user['id'], $device, $browser, $ip, $location]);
