...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['purchase_miner'])) {
        if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
            ...
            if ($package_id) {
                ...
                $stmt = $pdo->prepare("SELECT name, price, daily_profit, duration_days FROM mining_packages WHERE id = ?");
    ...
    $stmt = $pdo->prepare("SELECT SUM(mp.price) as total_investment FROM user_miners um JOIN mining_packages mp ON um.package_id = mp.id WHERE um.id = ?");
    $stmt->execute([$user_id]);
    ...
    $stmt = $pdo->prepare("SELECT SUM(mp.daily_profit) as daily_earnings FROM user_miners um JOIN mining_packages mp ON um.package_id = mp.id WHERE um.id = ? AND um.status = 'active'");
    $stmt->execute([$user_id]);
    ...
    // Fetch active miners
    $stmt = $pdo->prepare("SELECT um.id as miner_id, mp.name, mp.price, mp.daily_profit, um.purchase_date, um.status, um.days_remaining, mp.duration_days
                           FROM user_miners um JOIN mining_packages mp ON um.package_id = mp.id
                           WHERE um.id = ? ORDER BY um.purchase_date DESC LIMIT 5");
    $stmt->execute([$user_id]);
    ...
    $stmt = $pdo->prepare("SELECT id as package_id, name, price, daily_profit, daily_return_percentage, duration_days FROM mining_packages ORDER BY price ASC");
    $stmt->execute();
    ...
    $stmt = $pdo->prepare("SELECT DATE(created_at) as date,
                           ...
                           FROM transactions
                           WHERE id = ? AND created_at >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)
