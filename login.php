...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (empty($email) || empty($password)) {
            ...
            // Fetch user
            $stmt = $pdo->prepare("SELECT id, password_hash, full_name, referral_code, account_status FROM users WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch();
            ...
            if ($user && password_verify($password, $user['password_hash'])) {
                if ($user['account_status'] === 'suspended') {
                    ...
                    $location = 'Unknown';
                    $stmt->execute([$user['id'], $device, substr($browser, 0, 50), $ip, $location]);
